      ...
      {routines.length > 0 && filteredStudents.length > 0 && (
        <div className="print-area">
          ...
          {Array.from({ length: Math.ceil(filteredStudents.length / 2) }).map((_, pageIndex) => (
            <div key={pageIndex} className="break-after-page print:h-[297mm] print:w-[210mm] print:box-border print:p-[10mm] flex flex-col justify-between gap-2 bg-slate-100 p-8 my-8 print:bg-white print:my-0 shadow-lg print:shadow-none mx-auto max-w-[210mm]">
              ...
              {filteredStudents.slice(pageIndex * 2, pageIndex * 2 + 2).map((student, cardIndex) => (
                <div key={student.id} className="admit-card-container border-2 border-black bg-[#ffebe0] p-4 relative h-[120mm] flex flex-col font-serif" style={{ color: 'black' }}>
                  ...
                  <div className="flex justify-between mb-2 text-[10px] font-bold items-stretch">
                    ...
                    <div className="w-40 flex flex-col">
                      ...
                      <div className="flex-1 flex border border-black">
                        <div className="w-12 px-1 border-r border-black flex items-center leading-none">TIME :</div>
                        <div className="flex-1 px-1 flex items-center bg-white leading-none">{routines[0]?.exam_time || "10:10-12:10"}</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 border-2 border-black flex flex-col mb-4 bg-white text-xs font-bold">
                    ...
                    {Array.from({ length: Math.max(4, leftRoutine.length, rightRoutine.length) }).map((_, i) => (
